...
/** category 兼容两种形态：官方 getPageProperties 已归一化的 string
 * （getPageProperties.js:95-98 单选取第一个，'通用' 形态）与历史/测试数组形态。
 * string（非空）→ 单元素数组；数组 → 过滤字符串元素；其余 → []。 */
function toCategoryArray(v) {
  if (typeof v === 'string') {
    const t = v.trim()
    return t ? [t] : []
  }
...
function standardPage(page) {
  ...
  return {
    ...
    category: toCategoryArray(page.category),
