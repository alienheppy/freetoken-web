...
// QA 门禁（阻塞项 #3）：合并输入只允许两种官方发布状态——
// Published=代表行，Invisible=平台合并行（数据源）；Draft 及其它任何
// 状态（含空值）一律跳过，不得进入公开合并结果。
const ALLOWED_STATUSES = ['Published', 'Invisible']
...
function mergeModelPages(allPages) {
  ...
  for (const page of allPages) {
    ...
    if (!ALLOWED_STATUSES.includes(row.status)) continue
  ...
  return order
    ...
    // 公开门禁：每个 modelId 必须至少存在一条 Published 代表行；
    // Invisible 仅作为同 modelId 的合并输入（数据源），纯 Invisible
    // 模型（无代表行）不得进入公开合并结果。
    .filter(rows => rows.some(r => r.status === 'Published'))
    .map(rows => {
      const acc = reduceModel(rows)
